import { FREDOKA, buildXpHistory, buildCoverage, gaugeBackground, UNAIDED_RATE } from "../data";
    ...
    <div data-screen-label="Analytics" style={{ maxWidth: 1180, margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 22, marginBottom: 22 }} className="acad-analytics-top">
        ...
        <section style={{ ...card, padding: 24 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: "#9B7B5B" }}>This week · +1,780</span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 14, height: 190, paddingTop: 10 }}>
            {xpHistory.map((d) => (
              <div key={d.label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 7 }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: "#B0794A" }}>{d.valLabel}</div>
                <div style={{ height: d.height, width: "100%", maxWidth: 42, background: "repeating-linear-gradient(#6FBF73 0 14px,#5FAF63 14px 16px)", border: "3px solid #2E2620", borderRadius: 8 }} />
        ...
        <section style={{ ...card, padding: 24, display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <div style={{ position: "relative", width: 190, height: 190, borderRadius: "50%", background: gaugeBackground(UNAIDED_RATE), border: "4px solid #2E2620", display: "flex", alignItems: "center", justifyContent: "center", margin: "6px 0" }}>
            <div style={{ width: 120, height: 120, background: "#fff", border: "4px solid #2E2620", borderRadius: "50%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ fontFamily: FREDOKA, fontWeight: 700, fontSize: 38, lineHeight: 1, color: "#4C7A2F" }}>{UNAIDED_RATE}%</div>
      ...
      <section style={{ ...card, padding: 24 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {coverage.map((c) => (
            <div key={c.pattern} style={{ display: "flex", alignItems: "center", gap: 18 }} className="acad-coverage-row">
                ...
                {c.pegs.map((on, i) => (
                  <span
                    ...
                    style={{
                      ...
                      background: on ? c.litColor : "#E4D6B8",
                      boxShadow: on ? "inset 0 -3px 0 rgba(46,38,32,.15)" : "none",
                    }}
                  />
                ))}
              </div>
              <div style={{ fontSize: 12, fontWeight: 800, color: "#B0794A" }}>Lv {c.level}</div>
            </div>
          ))}
        </div>
      </section>
