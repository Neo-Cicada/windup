...
Tests run against a throwaway database: the schema is created once per session and every table is `TRUNCATE`d between tests, so the dev database is never touched. `asyncio_mode = "auto"` — do not add `@pytest.mark.asyncio`. Use the `client`, `db`, `seeded`, `auth` and `judge` fixtures from `tests/conftest.py` (`auth` returns a ready Bearer header). `auth2` / `judge2` are a second toy, which is what a duel test needs.

Because submitting is asynchronous, a test that wants a verdict has to run the judge itself: `judge.solve(slug)` submits, drains the queue and returns the settled result; `judge.submit(...)` / `judge.drain()` / `judge.result(id)` are there when the steps matter separately. The fixture runs `DispatchRunner`, which splits by language: Python borrows the host interpreter through `SubprocessRunner` (no artifact, and faster per run), and every other language goes to the real `WasmRunner` — so a Python-only test suite needs nothing fetched, while a test that judges Ruby, PHP or a compiled language **skips** unless that language's wasm is in `vendor/`.
...
- **`languages/__init__.py` is the registry**; `settings.JUDGE_LANGUAGES` is what a deployment actually offers. Registered and offered are different questions — a pack can exist while its artifact hasn't been fetched, and `get_pack` raises `UnknownLanguage` for both cases. Eight packs are registered today: Python, JavaScript, Ruby, PHP, SQL, C++, Rust and Go, with `DEFAULT_LANGUAGE` being Python — the language a problem falls back to and the one the workbench opens on.
...
- **`preambles.py` holds the structural harnesses**, one per language per shape (`REVERSE_LIST`, `CYCLE`, `TWO_LISTS`, `LIST_AND_INT`, `TREE`, `TWO_TREES`, `TREE_DUMP`), each paired with a `*_BENCHES` dict a problem can hand straight to `languages=`. `ALL_LANGUAGES` is the default and the common case.
...
- `components/RequireAuth.tsx` — mounted in `app/academy/layout.tsx`, so one gate covers all eleven routes. Children never render until `/me` succeeds, so there's no flash of the dashboard.
...
Styling is **inline style objects**, not CSS modules or a framework. `app/globals.css` carries only resets, base typography, the `@keyframes` (`cfall`, `spin`, `wob`, `floaty`, `pop`, `tick`) that components reference by name, and the media queries below. Shared tokens (`FREDOKA`, `DARK`, `MONO`, `NAV`) and pure presentation helpers (`buildShelves`, `pegColor`, `streakColors`, `buildPodium`) come from `components/academy/data.ts`; fonts are wired as CSS variables in `app/layout.tsx` via `next/font`.

An inline style can't hold a media query, so **everything responsive is a class in `globals.css` overriding those inline values** — hence the `!important`s, which answer inline specificity rather than fight another rule. Which layout you get is decided in CSS rather than by measuring the window, so the first paint is already right and there is nothing to mismatch on hydration. Three breakpoints, widest first: **960px** collapses the two-column screens (`.acad-dash`, `.acad-problem`, `.acad-analytics-top`, `.acad-acct-grid`); **900px** turns the sidebar into a top bar — brand and the `.acad-nav-toggle` menu button stay out, `.acad-nav` becomes the drawer that button opens, and since the sticky now belongs to that bar `.acad-topbar` gives up its own (two sticky headers would eat half a phone) and drops its meters onto a swipeable second line; **620px** tightens card padding, headings and list rows. Only `Sidebar` holds state for any of it (`open`, meaningless above 900px, put away by tapping a link — the layout doesn't unmount on navigation, so nothing else would close it).
