...
- **`languages/__init__.py` is the registry**; `settings.JUDGE_LANGUAGES` is what a deployment actually offers. Registered and offered are different questions — a pack can exist while its artifact hasn't been fetched. Today: Python, JavaScript, Ruby, PHP.
